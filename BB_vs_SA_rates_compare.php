...
include_once "update_BB_rates.php";

$BB_json_data = file_get_contents("json\\bb_todays_rate.json");
$BB_array = json_decode($BB_json_data, true);
...
echo "<th>ITEM</th>";
echo "<th>WEIGHT</th>";
echo "<th>BIG BASKET</th>";
echo "<th>SPICE ANGADI</th>";
echo "</tr>";


$array_keys = array_keys($BB_array);
sort($array_keys);

foreach ($array_keys as $mkey){

    $weight_keys = array_keys($BB_array[$mkey]);
    sort($weight_keys);

    foreach ($weight_keys as $wkey){

    # SPICE ANGADI rates lower than BIG BASKET..............

    if ($SA_array[$mkey][$wkey] < $BB_array[$mkey][$wkey]){
        ...
        echo "<td bgcolor='#15a331'>".$BB_array[$mkey][$wkey]."</td>";
        echo "<td bgcolor='#15a331'><span style='color:'>".$SA_array[$mkey][$wkey]."</span></td></tr>";
    }

    # SPICE ANGADI rates are higher than BIG BASKET.............

    else if($SA_array[$mkey][$wkey] > $BB_array[$mkey][$wkey]){
        ...
        echo "<td><font color='white'>".$BB_array[$mkey][$wkey]."</font></td>";
        echo "<td><span style='color:white'>".$SA_array[$mkey][$wkey]."</span></td></tr>";
    }

    # SPICE ANGADI and BIG BASKET have same rates for the item.............
